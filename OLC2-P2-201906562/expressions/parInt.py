...
class ParInt(Expression):
    ...
    def ejecutar(self, ast, env, gen, lvlbreak, lvlcont, lvlreturno):
        ...
        #     list = [f"El parserInt no acepta el tipo {StringType.Retorno(res.type.value)}", env.id, self.line, self.col, "Semantico"]
        #     ast.setErrors(list)
        #     return Symbol(line=self.line, col=self.col, value=None, type=Type.NULL)

        # try:
        #     Salida.value = int(res.value)
